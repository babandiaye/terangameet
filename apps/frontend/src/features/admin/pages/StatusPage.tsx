...
export const StatusPage = () => {
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    // The probe itself takes ~10s: the SMTP server delays its greeting banner
    // by 10s (Postfix postscreen). Polling every 10s meant re-probing as soon
    // as the previous round finished, hammering SMTP for nothing.
    refetchInterval: 30000,
    refetchOnWindowFocus: true,
  })

  // Deliberately not react-query's `isFetching`: that is also true for the 10s
  // background poll, so the button would blink on its own and a click would be
  // indistinguishable from the polling. This tracks the explicit action only.
  ...
  const allOk = data?.components.every(
  ...
  return (
    ...
    >
      ...
      {data && (
        ...
        >
          ...
          >
            ...
            >
              {/* `rotate` is the keyframe this design system defines; the
                  previous `spin` matched nothing, so the icon never turned and
                  the button looked dead while the probe ran. */}
              <RiRefreshLine
                size={15}
                aria-hidden="true"
                className={
      ...
      {isLoading ? (
        <div className={css({ color: 'greyscale.500' })}>
          Vérification des services…
        </div>
      ) : isError || !data ? (
        <div className={css({ color: 'danger.600' })}>
          Impossible de récupérer l’état des services.
        </div>
      ) : (
        <div
          className={css({
...
const ComponentCard = ({ c }: { c: ComponentHealth }) => {
  ...
  return (
    <Card>
      ...
      >
        <span className={css({ fontWeight: 700, fontSize: '0.98rem' })}>
          {c.label}
        </span>
        <span
          className={css({
