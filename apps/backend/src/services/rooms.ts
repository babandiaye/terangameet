...
export async function resolveRoom(roomId: string): Promise<{ room: Room | null; livekitRoom: string }> {
...
/**
 * Decide whether the caller may moderate the given room, using either a verified
 * LiveKit token (req.livekit) or the session user's role.
 */
export async function authorizeModeration(
  roomId: string,
  opts: { userId?: string | null; livekitRoom?: string; livekitIsAdmin?: boolean }
  ...
  return { ok: isAdminOrOwner(role), livekitRoom, isAdmin: isAdminOrOwner(role) }
}
...
export async function getRole(roomId: string, userId?: string | null): Promise<Role | null> {
...
export function liveKitRoomName(room: { id: string; slug: string | null } | { slug: string }): string {
...
export function serializeRoom(
  room: Pick<Room, 'id' | 'name' | 'slug' | 'accessLevel' | 'configuration' | 'pinCode'>,
  opts: SerializeOpts
) {
...
export function serializeEphemeralRoom(
  slug: string,
  opts: SerializeOpts
) {
  return {
...
export function publishableSources(config: RoomConfiguration | null | undefined): string[] {
