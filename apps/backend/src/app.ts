...
export function createApp() {
  ...
  // any of them guards ALL of them — including the public join and lobby routes
  // below. Guards belong on individual routes here, never on the router.
  ...
  if (env.SERVE_FRONTEND) {
    ...
    if (fs.existsSync(dist)) {
      ...
      app.get("*", (req, res, next) => {
        if (req.path.startsWith("/api") || req.path.startsWith("/oidc"))
          return next();
