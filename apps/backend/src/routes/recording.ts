...
import { resolveRoom, authorizeModeration, serializeRoom } from '../services/rooms'
...
recordingRoomRouter.post('/:roomId/start-recording/', async (req, res) => {
  ...
  const schema = z.object({
    mode: z.enum(['screen_recording', 'transcript']).default('screen_recording'),
  ...
  if (!room) return res.status(400).json({ detail: 'Recording requires a registered room.' })
  ...
  if (active) return res.status(409).json({ detail: 'A recording is already in progress.' })

  const mode = parsed.data.mode === 'transcript' ? 'TRANSCRIPT' : 'SCREEN_RECORDING'
  ...
  try {
    ...
    if (err instanceof Prisma.PrismaClientKnownRequestError && err.code === 'P2002') {
      return res.status(409).json({ detail: 'A recording is already in progress.' })
    }
    throw err
  }
  ...
  try {
    ...
    await roomService
      .updateRoomMetadata(livekitRoom, JSON.stringify({ recording_status: 'started', recording_mode: parsed.data.mode }))
      .catch((e) => logger.warn(`[recording] metadata update (start) failed: ${(e as Error).message}`))
    await notifyRoom(livekitRoom, { type: 'screenRecordingStarted' })
  } catch (err) {
    await prisma.recording.update({ where: { id: recording.id }, data: { status: 'FAILED_TO_START' } })
...
recordingRoomRouter.post('/:roomId/stop-recording/', async (req, res) => {
  ...
  try {
    ...
    await prisma.recording.update({ where: { id: active.id }, data: { status: 'STOPPED' } })
    ...
    const modeStr = active.mode === 'TRANSCRIPT' ? 'transcript' : 'screen_recording'
    await roomService
      .updateRoomMetadata(livekitRoom, JSON.stringify({ recording_status: 'saving', recording_mode: modeStr }))
      .catch((e) => logger.warn(`[recording] metadata update (stop) failed: ${(e as Error).message}`))
...
recordingRoomRouter.post('/:roomId/start-subtitle/', async (req, res) => {
  if (!env.subtitle.enabled) return res.status(403).json({ detail: 'Subtitles are disabled.' })
  const { room, livekitRoom } = await resolveRoom(req.params.roomId)
  const auth = await authzAdmin(req, req.params.roomId)
  if (!auth.ok) return res.status(403).json({ detail: 'Insufficient privileges.' })
...
recordingsRouter.get('/', async (req, res) => {
  ...
  const [count, recordings] = await Promise.all([
    ...
    prisma.recording.findMany({ where, orderBy: { createdAt: 'desc' }, skip, take }),
  ])
  res.json(paginated(count, page, pageSize, recordings.map(serializeRecording)))
})
...
recordingsRouter.get('/:id/media/', async (req, res) => {
  ...
  const ext = r.mode === 'TRANSCRIPT' ? 'ogg' : 'mp4'
  try {
    const obj = await getObjectStream(recordingKey(r), env.recording.bucket)
    if (!obj.body) return res.status(404).json({ detail: 'Recording file not found.' })
    res.setHeader('Content-Type', obj.contentType || (ext === 'ogg' ? 'audio/ogg' : 'video/mp4'))
    if (obj.contentLength != null) res.setHeader('Content-Length', String(obj.contentLength))
    res.setHeader('Content-Disposition', `attachment; filename="${r.id}.${ext}"`)
...
recordingsRouter.delete('/:id', async (req, res) => {
  const r = await prisma.recording.findFirst({
    where: { id: req.params.id, accesses: { some: { userId: req.user!.id, role: { in: ['OWNER', 'ADMIN'] } } } },
  })
  if (!r) return res.status(404).json({ detail: 'Recording not found.' })
  const final = ['STOPPED', 'SAVED', 'ABORTED', 'FAILED_TO_START', 'FAILED_TO_STOP', 'NOTIFICATION_SUCCEEDED']
  if (!final.includes(r.status)) {
    return res.status(409).json({ detail: 'Cannot delete a recording that is not finished.' })
  }
